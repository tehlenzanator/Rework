...
for (var creep of Object.values(Game.creeps)) {
  ...
  if(memory.role == "Runners") {roleRunner.run(creep);} //Do this for all the roles
  if(memory.role == 'superHarvester') {rolesuperHarvester.run(creep);}
    }
function spawning(Runner, superHarvesters, Upgraders) { //Only pass what you need until you need it
    const spawn = Game.spawns['Spawn1']; //Localize
    if (!spawn.spawning) {
      if (runners.length < 2) {
          let newName = 'Runners' + Game.time;
          console.log('Spawning new runner:', newName);
          spawn.spawnCreep([WORK, CARRY, MOVE], newName, {memory: {role: 'Runners'}});
